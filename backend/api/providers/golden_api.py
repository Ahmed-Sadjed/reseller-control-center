...
class GoldenAPIAdapter(BaseProviderAdapter):
    ...
    def _request(self, method, path, **kwargs):
        ...
        try:
            ...
            if response.status_code == 422:
                ...
                error = ProviderAPIError(f"Validation Error (422): {err_msg}")
                error.code = 422
                raise error
    ...
    def create(self, pack_id: int, months: int, is_lifetime: bool = False, username='', password='', note='', **kwargs) -> dict:
        base_username = username or f"golden_{uuid.uuid4().hex[:8]}"
        final_password = password or uuid.uuid4().hex[:8]
        ...
        for attempt in range(max_attempts):
            if attempt > 0:
                current_username = f"{base_username}_{uuid.uuid4().hex[:4]}"
        ...
        if exp_date_str:
            try:
                dt = datetime.strptime(exp_date_str, "%Y-%m-%d").replace(tzinfo=timezone.utc)
